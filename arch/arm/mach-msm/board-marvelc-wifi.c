/* linux/arch/arm/mach-msm/board-marvelc-wifi.c
*/
...
int marvelc_wifi_power(int on);
int marvelc_wifi_reset(int on);
int marvelc_wifi_set_carddetect(int on);
...
static void *marvelc_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init marvelc_init_wifi_mem(void)
{
	int i;
...
static struct platform_device marvelc_wifi_device = {
	.name           = "bcm4329_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(marvelc_wifi_resources),
	.resource       = marvelc_wifi_resources,
	.dev            = {
		.platform_data = &marvelc_wifi_control,
	},
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned marvelc_wifi_update_nvs(char *str)
{
