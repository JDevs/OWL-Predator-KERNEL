...
enum {
        SUPERBAD_DEBUG_JUMPS=1,
        SUPERBAD_DEBUG_LOAD=2
};
...
// policy->max > sleep_max_freq ? (sleep_max_freq > policy->min ? sleep_max_freq : policy->min) : policy->max;
                ...
                this_superbad->max_speed = policy->max;
        }
}

inline static unsigned int validate_freq(struct superbad_info_s *this_superbad, int freq) {
        if (freq > this_superbad->max_speed)
                return this_superbad->max_speed;
        if (freq < this_superbad->min_speed)
                return this_superbad->min_speed;
        return freq;
}

static void reset_timer(unsigned long cpu, struct superbad_info_s *this_superbad) {
  this_superbad->time_in_idle = get_cpu_idle_time_us(cpu, &this_superbad->idle_exit_time);
...
unsigned long new_rate;
        ...
        if (this_superbad->idle_exit_time == 0 || update_time == this_superbad->idle_exit_time)
                return;

        delta_idle = cputime64_sub(now_idle, this_superbad->time_in_idle);
        delta_time = cputime64_sub(update_time, this_superbad->idle_exit_time);
        //printk(KERN_INFO "superbadT: t=%llu i=%llu\n",cputime64_sub(update_time,this_superbad->idle_exit_time),delta_idle);

        // If timer ran less than 1ms after short-term sample started, retry.
        if (delta_time < 1000) {
                if (!timer_pending(&this_superbad->timer))
        ...
        if (debug_mask & SUPERBAD_DEBUG_LOAD)
                printk(KERN_INFO "superbadT @ %d: load %d (delta_time %llu)\n",policy->cur,cpu_load,delta_time);

        this_superbad->cur_cpu_load = cpu_load;

        // Scale up if load is above max or if there where no idle cycles since coming out of idle,
        // or when we are above our max speed for a very long time (should only happend if entering sleep
        // at high loads)
        if ((cpu_load > max_cpu_load || delta_idle == 0) &&
            !(policy->cur > this_superbad->max_speed &&
              cputime64_sub(update_time, this_superbad->freq_change_time) > 100*down_rate_us)) {

                if (policy->cur > this_superbad->max_speed) {
                        reset_timer(data,this_superbad);
                }
...
static void cpufreq_idle(void)
{
        ...
        if (!this_superbad->enable) {
                pm_idle_old();
                return;
        }

        if (policy->cur == this_superbad->min_speed && timer_pending(&this_superbad->timer))
...
static void cpufreq_superbad_freq_change_time_work(struct work_struct *work)
{
        ...
        for_each_cpu(cpu, tmp_mask) {
                ...
                cpu_load = this_superbad->cur_cpu_load;
                force_ramp_up = this_superbad->force_ramp_up && nr_running() > 1;
...
// imoseyon - should only register for suspend when governor active
        ...
        case CPUFREQ_GOV_LIMITS:
                superbad_update_min_max(this_superbad,new_policy,suspended);
                if (this_superbad->cur_policy->cur != this_superbad->max_speed) {
                        if (debug_mask & SUPERBAD_DEBUG_JUMPS)
                                printk(KERN_INFO "SmartassI: initializing to %d\n",this_superbad->max_speed);
                        __cpufreq_driver_target(new_policy, this_superbad->max_speed, CPUFREQ_RELATION_H);
                }
                break;
...
rapid_min_cpu_load = RAPID_MIN_CPU_LOAD;
        ...
        for_each_possible_cpu(i) {
                ...
                this_superbad->enable = 0;
                this_superbad->cur_policy = 0;
                this_superbad->force_ramp_up = 0;
                this_superbad->max_speed = DEFAULT_SLEEP_WAKEUP_FREQ;
                this_superbad->min_speed = DEFAULT_AWAKE_MIN_FREQ;
                this_superbad->time_in_idle = 0;
                this_superbad->idle_exit_time = 0;
                this_superbad->freq_change_time = 0;
                this_superbad->freq_change_time_in_idle = 0;
                this_superbad->cur_cpu_load = 0;
                // intialize timer:
...
down_wq = create_workqueue("ksmartass_down");
        ...
        pr_info("[imoseyon] superbad enter\n");
...
{
        pr_info("[imoseyon] superbad exit\n");
