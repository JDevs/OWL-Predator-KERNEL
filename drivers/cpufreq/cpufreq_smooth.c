/*
 *  drivers/cpufreq/cpufreq_smooth.c
 *
 *  Copyright (C)  2001 Russell King
 *            (C)  2003 Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>.
 *                      Jun Nakajima <jun.nakajima@intel.com>
 *            (C)  2009 Alexander Clouter <alex-kernel@digriz.org.uk>
 *
...
#define MIN_SAMPLING_RATE			\
			(def_sampling_rate / MIN_SAMPLING_RATE_RATIO / 2)
#define MAX_SAMPLING_RATE			(100 * def_sampling_rate)
...
/* cpufreq_smooth Governor Tunables */
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SMOOTH
static
#endif
struct cpufreq_governor cpufreq_gov_smooth = {
	.name			= "smooth",
...
static void smooth_early_suspend(struct early_suspend *handler) {
	suspended = 1;
}

static void smooth_late_resume(struct early_suspend *handler) {
	suspended = 0;
}

static struct early_suspend smooth_power_suspend = {
	.suspend = smooth_early_suspend,
	.resume = smooth_late_resume,
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
};

static int __init cpufreq_gov_dbs_init(void)
{
	register_early_suspend(&smooth_power_suspend);
	return cpufreq_register_governor(&cpufreq_gov_smooth);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	unregister_early_suspend(&smooth_power_suspend);
	cpufreq_unregister_governor(&cpufreq_gov_smooth);
}


MODULE_AUTHOR ("Emilio LÃ³pez <turl@tuxfamily.org>");
MODULE_DESCRIPTION ("'cpufreq_smooth' - A dynamic cpufreq governor for "
		"Low Latency Frequency Transition capable processors "
		"optimised for use in a battery environment"
		"Based on conservative by Alexander Clouter");
MODULE_LICENSE ("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SMOOTH
