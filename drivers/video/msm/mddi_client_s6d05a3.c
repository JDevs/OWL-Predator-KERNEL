...
static int samsung_recover(struct msm_panel_data *panel_data)
{
	struct panel_info *panel = container_of(panel_data, struct panel_info,
						panel_data);
	struct msm_mddi_client_data *client_data = panel->client_data;
...
static int mddi_samsung_probe(struct platform_device *pdev)
{
	...
	panel->panel_data.recover_vsync = samsung_recover;
...
static struct platform_driver mddi_client_0101_05a3 = {
	.probe = mddi_samsung_probe,
	.remove = mddi_samsung_remove,
	.driver = { .name = "mddi_c_0101_05a3" },
};

static int __init mddi_client_samsung_init(void)
{
	return platform_driver_register(&mddi_client_0101_05a3);
}
